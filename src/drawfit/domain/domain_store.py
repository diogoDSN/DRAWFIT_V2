...
class DomainStore:
    ...
    def changeLeagueCode(self, leagueName: str, site: Sites, newCode: str):
    ...
    def updateLeaguesOdds(self, results: Dict[str, List[List[OddSample]]]) -> Dict:

        to_update = {}

        for league_id, odds_sample in results.items():
            league = next(league for league in self.knownLeagues if league.name == league_id)
            to_update[league_id] = league.updateOdds(odds_sample)
